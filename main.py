...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        print("Epoch #: {}".format(epoch + 1), "/ {}".format(epochs), " Loss: {:.2f}".format(loss), " Time: ",
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, training_operation, cross_entropy_loss = optimize(model, correct_label, learning_rate, num_classes)

        # TODO: Train NN using the train_nn function
        tf.set_random_seed(123)
        sess.run(tf.global_variables_initializer())
        print("Starting traning with {} epochs".format(EPOCHS))
